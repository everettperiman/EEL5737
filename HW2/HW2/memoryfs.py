...
### Start of new code ###
  ...
  def Link(self, target, name, cwd):
    ...
    if target_inode_num == -1:
      logging.debug ("Link: Target node does not exist")
      return -1
    ...
    if dir_inode.inode.type != INODE_TYPE_DIR:
      logging.debug("Link: The cwd is not valid")
      return -1

    fileentry_position = self.FindAvailableFileEntry(cwd)
    if fileentry_position == -1:
      logging.debug("Link: No extra entrys available")
      return -1

    if self.Lookup(name, cwd) != -1:
      logging.debug("Link: Name already exists in " + str(cwd))
      return -1

    target_inode_obj = InodeNumber(self.RawBlocks, target_inode_num)
    target_inode_obj.InodeNumberToInode()

    if target_inode_obj.inode.type != INODE_TYPE_FILE:
      logging.debug ("Link: Target is not a file")
      return -1

    self.InsertFilenameInodeNumber(dir_inode, name, target_inode_num)

    target_inode_num_obj = InodeNumber(self.RawBlocks, target_inode_num)
    target_inode_num_obj.InodeNumberToInode()
    target_inode_num_obj.inode.refcnt += 1
    target_inode_num_obj.StoreInode()

    return 0
   

  def Chroot(self, chrootpath, cwd):
    # This changes the root inode such that if succesfull all future ref to / are resolved through chrootpath
    # Chrootpath must resovle as a general path and must resolve toa  dir type inode for chroot to ex succesfully
    # It is okay if you can escape the chroot with relative paths
    chroot_inode = self.GeneralPathToInodeNumber(chrootpath, cwd)

    chroot_inode_obj = InodeNumber(self.RawBlocks, chroot_inode)
    chroot_inode_obj.InodeNumberToInode()

    if chroot_inode_obj.inode.type != INODE_TYPE_DIR:
      logging.debug("Link: The cwd is not valid")
      return -1

    # Check if chroot_inode is a directory
    self.rootdir = chroot_inode
    return 0
    

  def PathToInodeNumer(self, path, dir):

    if(self.PlainName(path)):
      return self.NameToInodeNumber(path, dir)
    
    else:
      # If we are looking at an absolute path remove the root reference     
      # Grab the first object in the path

      split_path = path.split("/")
      first = split_path[0]
      dir = self.Lookup(first, dir)

      split_rest = split_path[1:]
      rest = "/".join(split_rest)
      return self.PathToInodeNumer(rest, dir)
  ...
  def GeneralPathToInodeNumber(self, path, cwd):
    ...
    if(path[0] == "/"):
      ...
      path = path[1:] # Omit the root part of the pathname
      return self.PathToInodeNumer(path, self.rootdir)
    
    else:
      return self.PathToInodeNumer(path, cwd)
...
## returns number of bytes written
  ...
  def Write(self, file_inode_number, offset, data):
    ...
    if offset > file_inode.inode.size:
      ...
      print("Write: offset larger than file size " + str(file_inode.inode.size))
      return -1

    if offset + len(data) > MAX_FILE_SIZE:
      logging.debug ("Write: exceeds maximum file size: " + str(MAX_FILE_SIZE))
      print("Write: exceeds maximum file size: " + str(MAX_FILE_SIZE))
      return -1
  ...
  def Read(self, file_inode_number, offset, count):
    ...
    # Set a maximum value to return
    tail = offset + count

    # Check if the inode number is valid
    if file_inode_number not in range(MAX_NUM_INODES):
      print("Error: not a valid Inode")
      return -1

    file_inode_obj = InodeNumber(self.RawBlocks, file_inode_number)
    file_inode_obj.InodeNumberToInode()
    file_blocks = file_inode_obj.inode.block_numbers
    file_blocks = [block for block in file_blocks if block]

    # Check if the offset is greater than the file size
    if offset > BLOCK_SIZE * len(file_blocks):
      print("Error: offset exceeds file size")
      return -1

    # Restrict the tail from being longer than the file
    if tail > BLOCK_SIZE * len(file_blocks):
      tail == BLOCK_SIZE * len(file_blocks)
    ...
    # Collect of the file block strings
    for block in file_blocks:
      base_string = base_string + self.RawBlocks.Get(block).decode(encoding='UTF-8',errors='ignore')

    # Return desired range of file information
    return bytearray(base_string[offset:tail], 'utf-8')
