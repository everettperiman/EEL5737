...
class DiskBlocks():
  def __init__(self, args):
    ...
    # Keeps track of if there are enough good servers
    self.good_servers = 0

    # Holds the SERVER objects
    self.serverlookup = []

    # Holds the xmlrpc objects
    self.servers = []

    args_dict = vars(args)

    # Iterate through all remaining args
    for argument in args_dict:

      # Check if the arg is port related
      if "port" in argument:

        # Check if the arg has a valid value
        if(args_dict[argument]):

          # Build the xmlrpc server url
          ...
          # Build the SERVER object class items and the list to hold them
  ...
  def byte_xor(self, arrayA, arrayB):
    ...
    # Iterate over the bytearrays and xor the bytes one by one
  ...
  def ConvertVblockData(self, vblock):
    # Calculate the stripe of the physical block
    stripe = vblock // (self.nservers-1)

    # Calculate which server in the stripe is responsible for the parity
    parityserver = (self.nservers - 1) - (stripe % (self.nservers))

    # Calculate which server has the physical block
    server = vblock % (self.nservers-1) 

    # Prevent collisions by iterating the server number if it is greater than the parity server
...
## Copy a block from the cache, or bring a block from server and add to the cache
  ...
  def Get(self, block_number):
    ...
    if block_number in range(0,TOTAL_NUM_BLOCKS):
      ...
      server, stripe, parityserver = self.ConvertVblockData(block_number)
      data = self.ServerGet(server, stripe)
